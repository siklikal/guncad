...
create table if not exists public.payments (
  ...
  payment_type text not null check (payment_type in ('model')),
...
comment on column public.payments.payment_type is 'model = one-time model purchase';
...
create table if not exists public.downloads (
  ...
  model_title text,
  download_type text not null check (download_type in ('purchased', 'free')),
...
comment on column public.downloads.download_type is 'purchased = bought individually, free = free model';
